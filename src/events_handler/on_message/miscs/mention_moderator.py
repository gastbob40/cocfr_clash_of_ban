...
async def mention_moderator(client: discord.Client, message: discord.Message, config):
    ...
    for role in message.role_mentions:
        if role.id == 490155585371766814:
            barbar_role: discord.Role = message.guild.get_role(280457976719081491)

            if not barbar_role in message.author.roles:
                return await message.channel.send(
                    embed=EmbedsManager.error_embed(
                        f"Erreur, il faut être niveau 5 (soit {barbar_role.name}) afin de mentionner un modérateur.\n"
